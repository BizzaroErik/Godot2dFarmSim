...
#Tau is pie
const GAME_MINUTE_DUTATION: float = TAU / MINUTES_PER_DAY
...
func _process(delta: float) -> void:
	time += delta * game_speed * GAME_MINUTE_DUTATION
	game_time.emit(time)
	recalculate_time()

func recalculate_time() -> void:
	var total_min = int(time / GAME_MINUTE_DUTATION)
...
func set_initial_time() -> void:
	...
	time = initial_total_minutes * GAME_MINUTE_DUTATION
